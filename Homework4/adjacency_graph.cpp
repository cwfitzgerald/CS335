...
void AdjacencyGraph::add_edge(std::size_t const start, std::size_t const end, float const weight) {
    ...
    auto itr = std::lower_bound(adjacency_.begin(), adjacency_.end(), adj, [&](Adj left, Adj right) {
        return ss_pair(left.start, left.end) < ss_pair(right.start, right.end);
    });
	// Prevent duplicates
	if (itr == adjacency_.end() || *itr != adj) {
		adjacency_.insert(itr, adj);
	}
}
...
void AdjacencyGraph::remove_edge(std::size_t start, std::size_t end) {
    ...
    auto itr = std::lower_bound(adjacency_.begin(), adjacency_.end(), pair, [&](Adj left, ss_pair right) {
        return ss_pair(left.start, left.end) < right;
    });
    assert(itr != adjacency_.end());
    adjacency_.erase(itr - 1);
}

std::vector<Adj> AdjacencyGraph::adjacents(std::size_t const idx) {
    auto const lower = std::lower_bound(adjacency_.begin(), adjacency_.end(), idx, [](Adj const& lhs, std::size_t rhs) {
        return lhs.start < rhs;
    });
    auto const upper = std::upper_bound(adjacency_.begin(), adjacency_.end(), idx, [](std::size_t lhs, Adj const& rhs) {
